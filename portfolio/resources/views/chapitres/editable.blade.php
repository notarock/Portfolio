	...
	<div class="form-group">
		...
		<img class="img-responsive img-responsive-centere" style="width:40%; padding:15 0;" src="/img/{{$chapitre->projet_id}}/{{$chapitre->picture}}" alt="Image du chapitre">
		<input id="picture" type="file" name="picture"  value="{{$chapitre->picture or old('picture')}}">
		<p class="help-block">Choisir une image</p>
	</div>

	<div class="form-group">
		<label for="textUp">Text en haut de la photo:</label>
		<textarea id="textUp" class="form-control" placeholder="Présentation du chapitre..." name="textUp">{{$chapitre->textUp or old('textUp')}}</textarea>    
	</div>

	<div class="form-group">
		<label for="textDown">Text sous la photo:</label>
		<textarea id="textDown" class="form-control" placeholder="Explication de la photo..." name="textDown">{{$chapitre->textDown or old('textDown')}}</textarea>    
	</div>
